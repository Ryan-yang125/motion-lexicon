...
export function MethodPage({ locale }: { locale: Locale }) {
  const labels = locale === "zh"
    ? {
        ...
        intro: "Motion Lexicon 把原子动效与完整 React 组件放在同一套开放内容系统里。每一条内容都应能解释用途、展示状态、给出可复制实现，并保留减弱动效方案。",
        method: "内容方法",
        source: "来源与许可",
        maintenance: "维护与贡献",
    ...
    : {
        ...
        intro: "Motion Lexicon keeps motion primitives and complete React components in one open content system. Every entry should explain use, show state, provide copy-ready implementation, and retain a reduced-motion treatment.",
  ...
  const sections = locale === "zh"
    ? [
        ["从真实状态开始", "组件用开始、进行中、完成和恢复等状态组织一段交互；原子动效拆出进入、节奏、连续性和反馈等可复用行为。"],
    ...
    : [
        ["Start from real state", "Components organize an interaction through start, progress, completion, and recovery. Primitives isolate reusable behaviours such as arrival, timing, continuity, and feedback."],

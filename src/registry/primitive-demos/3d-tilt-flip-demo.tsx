...
function Face({ back, locale }: { back?: boolean; locale: "zh" | "en" }) {
  return <div className={`size-full rounded-[13px] border p-4 shadow-[0_8px_20px_-16px_rgba(28,25,23,0.55)] ${back ? "border-stone-800 bg-stone-800 text-white dark:border-stone-100 dark:bg-stone-100 dark:text-stone-900" : "border-stone-200 bg-white dark:border-white/[0.12] dark:bg-[#20201D]"}`}><div className="flex items-center justify-between"><span className="text-[9px] uppercase tracking-[0.08em] opacity-50">Workspace</span><StatusPill>{back ? "PRO" : "FREE"}</StatusPill></div><strong className="mt-8 block text-[16px]">{back ? textFor(locale, "团队计划", "Team plan") : "Motion Lexicon"}</strong><p className="mt-1 text-[10px] opacity-55">{back ? textFor(locale, "无限项目 · 12 位成员", "Unlimited projects · 12 seats") : textFor(locale, "点击查看计划", "Flip to view plan")}</p></div>;
}

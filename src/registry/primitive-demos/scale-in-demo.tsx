...
export function ScaleInDemo({ locale, values, compact, replayKey }: PrimitiveDemoProps) {
  ...
  const items = locale === "zh" ? ["画框", "页面", "流程"] : ["Frame", "Page", "Flow"];
  return (
          ...
          >
            <ProductPanel className="p-2">
              ...
              <div className="grid grid-cols-3 gap-1.5 p-2">
                {items.map((item) => <button className="rounded-[8px] bg-stone-50 px-1 py-3 text-[10px] text-stone-500 dark:bg-white/[0.05] dark:text-stone-400" key={item}>{item}</button>)}
              </div>
            </ProductPanel>
          </ScaleInPrimitive>

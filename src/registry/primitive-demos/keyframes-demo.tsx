...
export function KeyframesDemo({ locale, values, compact, replayKey }: PrimitiveDemoProps) {
  const milestones = locale === "zh" ? ["触发", "移动", "收稳"] : ["Trigger", "Move", "Settle"];
  return (
      ...
      <div className="grid h-full place-items-center">
        <ProductPanel className="w-full max-w-[340px] px-4 py-5">
          ...
          <div className="relative h-12 border-t border-stone-200 dark:border-white/[0.12]">
            {milestones.map((label, index) => <span className="absolute top-[-4px] grid gap-2 text-[9px] text-stone-400" style={{ left: `${index * 50}%`, transform: index === 2 ? "translateX(-100%)" : undefined }} key={label}><i className="size-2 rounded-full bg-stone-300 dark:bg-stone-600" />{label}</span>)}
            <KeyframesPrimitive
              key={replayKey}

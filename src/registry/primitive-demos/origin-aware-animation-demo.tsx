      ...
      <div className="relative h-full">
        <div className="absolute bottom-2 left-2">
          ...
          <OriginAwareAnimationPrimitive open={open} origin={stringValue(values, "origin", "top-left") as MotionOrigin} startScale={numberValue(values, "scale", 88) / 100} duration={numberValue(values, "duration", 220) / 1000} easing={easingValue(values, "ease", "soft")} className="absolute bottom-11 left-0 w-[190px]">
            <ProductPanel className="p-2"><strong className="block px-2 py-1.5 text-[10px]">{textFor(locale, "添加图层", "Add layer")}</strong>{["Frame", "Text", "Image"].map((item) => <button className="block min-h-7 w-full rounded-[7px] px-2 text-left text-[10px] text-stone-500 hover:bg-stone-50 dark:hover:bg-white/[0.05]" key={item}>{item}</button>)}</ProductPanel>

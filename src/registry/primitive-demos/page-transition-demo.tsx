...
export function PageTransitionDemo({ locale, values, compact }: PrimitiveDemoProps) {
  ...
  const pages = [
    { name: textFor(locale, "概览", "Overview"), tone: "bg-[#4568FF]" },
    { name: textFor(locale, "动态", "Activity"), tone: "bg-[#93664F]" },
    { name: textFor(locale, "文件", "Files"), tone: "bg-[#55745D]" },
  ];
  ...
  return (
    <PrimitiveDemoSurface label={textFor(locale, "项目空间", "Project space")} meta={current.name} compact={compact}>
      <ProductPanel className="flex h-full flex-col overflow-hidden">
        ...
        <div className="min-h-0 flex-1 overflow-hidden p-3"><PageTransitionPrimitive pageKey={page} direction={stringValue(values, "direction", "left") as PageTransitionDirection} distance={numberValue(values, "distance", 32)} duration={numberValue(values, "duration", 280) / 1000} easing={easingValue(values, "ease", "soft")} className="h-full"><div className={`h-full rounded-[10px] p-4 text-white ${current.tone}`}><span className="text-[9px] opacity-65">{textFor(locale, "项目 / 04", "PROJECT / 04")}</span><strong className="mt-5 block text-[15px]">{current.name}</strong><p className="mt-1 text-[10px] opacity-70">{textFor(locale, "视图内容保持连续进入。", "The next view arrives with continuity.")}</p></div></PageTransitionPrimitive></div>

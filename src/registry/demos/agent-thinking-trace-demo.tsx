...
export function AgentThinkingTraceDemo({ locale = "en" }: DemoLocaleProps = {}) {
  ...
  return <div role="group" aria-label={demoText("agent-thinking-trace", locale)} className="mx-auto w-full max-w-[430px] rounded-[16px] bg-[#edf4f1] p-3 shadow-[0_18px_38px_-30px_rgba(17,59,48,.65)]"><AgentThinkingTrace steps={steps} label={demoValue(locale, "正在规划界面", "Planning the interface")} duration={`${(active + 1) * 1.7}s`} /></div>;
}

...
export function ThemeRevealDemo({ locale = "en" }: DemoLocaleProps = {}) {
  ...
  return (
    <div role="group" aria-label={demoText("theme-reveal", locale)} data-theme-reveal-state={theme} className={`w-full max-w-[420px] rounded-[16px] border p-4 transition-[background-color,border-color,color] duration-200 ${dark ? "border-white/10 bg-[#1D1D1A] text-stone-100" : "border-stone-200 bg-[#ECE8E0] text-stone-800"}`}>

...
describe("Motion Grammar", () => {
  it("keeps the two content collections complete and equally addressable", () => {
    ...
    expect(motionGrammar.collections.moments.count).toBe(motionPacks.length);
    expect(motionGrammar.collections.primitives.count).toBe(44);
    expect(motionGrammar.collections.moments.count).toBe(28);
  });
  ...
  it("exposes a stable five-mode director workflow", () => {
    ...
    expect(motionGrammarDataPath).toBe("/data/v2/motion-grammar.json");
  });
  ...
  it("grounds the public blueprint in existing motion content", () => {
    ...
    expect(motionBlueprintExample.provenance.relatedPacks.every((id) => packIds.has(id))).toBe(true);
  });

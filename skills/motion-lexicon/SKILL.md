...
4. Return the CLI-provided `compareUrl` unchanged. This compatibility field opens the Finder with one primary preview, three ranked static candidate choices, and replay for the current candidate. Explain the meaningful choice in the user's language.
...
8. Return the original intent, chosen variant and canonical recipe IDs, rationale, live preview URL, Finder URL from `compareUrl` when applicable, chosen parameter values, reduced-motion treatment, and requested output.
...
- Return `compareUrl` unchanged so the user receives the same ordered Top 3 and current primary selection in the web Finder.
